...
Y = Get_slab_analytical(Thermal_cond,L,N_points_interior,Surrounding_temp, T_init);
...
function [array_temps_analytical] = Get_slab_analytical(K,L,N_points,T_surr, T_b)
    ...
    for i = 2:1:N_points+2
        array_temps_analytical(i,1) = array_temps_analytical(i-1) + ((T_surr-T_b)/L)*delta_x(i);
...
for i = 2:1:N_points+1
    % disp(i);
    d(i) = -(1/(delta_x(i-1)) + 1/delta_x(i+1) + m_squared); % here's where St comes in use
    % disp(d(i));
    a(i) = (1/delta_x(i+1));
    b(i) = (1/delta_x(i-1));
    c(i) = -m_squared*T_surr; % here's where Sc comes in use
end
