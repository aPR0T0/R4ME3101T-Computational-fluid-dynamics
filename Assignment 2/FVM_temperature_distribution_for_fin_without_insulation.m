...
Conv_coefficient = 0;
Surrounding_temp = 20;
Z = Get_slab_temperature(Thermal_cond,Q_flux, T_init, Sc, St, N_points_interior, L, area_of_crossection,Conv_coefficient, Perimeter, Surrounding_temp);
Y = Get_slab_analytical(5,L,N_points_interior,Surrounding_temp, T_init);
...
function [array_temps_analytical] = Get_slab_analytical(m,L,N_points,T_surr, T_b)
    ...
    for i = N_points+2:-1:1
        array_temps_analytical(i,1) = T_surr + (T_b - T_surr)*((cosh(m*(L-x(i)))+sinh(m*(L-x(i))))/(cosh(m*L)+sinh(m*L)));
...
function [array_temps] = Get_slab_temperature(Thermal_cond,Q_flux, T_init, Sc, St, N_points, L, area, h, p, T_surr)
