...
Y = Get_slab_analytical(m,L,N_points_interior,Surrounding_temp, T_init);
...
function [array_temps_analytical] = Get_slab_analytical(m,L,N_points,T_surr, T_b)
    ...
    for i = N_points+2:-1:1
        array_temps_analytical(i,1) = T_surr + (T_b - T_surr)*cosh(m*(L-x(i)))/cosh(m*L);
